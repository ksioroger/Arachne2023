...
timeTrace(const char* format, uint64_t arg0 = 0, uint64_t arg1 = 0,
          uint64_t arg2 = 0, uint64_t arg3 = 0, uint64_t arg4 = 0) { //Incluindo o arg4 para permitir passar o tempo de preempção no arg3 - Including arg4 to allow passing preemption time on arg3
#if TIME_TRACE
 ...
 * /////////////////////////////////////////////////////////////////////////////////////
 * \param timeToPreempt
 *     Um inteiro que define um tempo limite para uma thread executar em segundos.
 *     Caso o tempo de execução da thread em execução extrapole este tempo a thread
 *     será preemptada e o processo terá suas demais threads(caso haja) impedidas 
 *     de ser escalonadas, para que outro processo tenha acesso ao núcleo.
 * /////////////////////////////////////////////////////////////////////////////////////
...
CoreArbiterServer::CoreArbiterServer(std::string socketPath,
    ...
    : socketPath(socketPath),
      ...
      timeToPreempt(static_cast<uint64_t>(timeToPreempt) * 1000000000), //Converte o tempo em segundos(int) para nanosegundos(uint64_t) - Converts time in seconds(int) to nanoseconds(uint64_t)
      timeToCleanUM(0) {    // Gatilho para disparar a limpeza do mapa de threads em execução - Trigger to trigger cleaning of running thread map
    if (sys->geteuid()) {
...
CoreArbiterServer::handleEvents() {
    ...
    for (int i = 0; i < numFds; i++) {
        ...
        if (events[i].events & EPOLLRDHUP) { //EPOLRDHUP Event no socket detectado
        ...
        } else if (socket == listenSocket) { //Nova Thread Se conectando
            // A new thread is connecting
            acceptConnection(listenSocket);
        } else if (timerFdToInfo.find(socket) != timerFdToInfo.end()) { //O Socket está associado a um temporizador
        ...
        } else if (socket == terminationFd) { // Tratamento de eventos em handleEvents() deve terminar
            return false;
        } else {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 * Realiza a preempção de uma thread 
 * 
 * Preempts a long thread
*/
void CoreArbiterServer::ThreadPreemption_portempo(ThreadInfo* thread) {
    struct ProcessInfo* process = thread->process;
    timeTrace("SERVER: Timing out thread preemption reached");
    /**
     * Verifica se a thread já liberou o núcleo
     * 
     * Checks if the thread has already freed the core
    */
    if (!thread) {
        /**
         * Esse processo já liberou o core que deveria
         * 
         * This process has already released the core that should
        */
        //LOG(DEBUG, "O gatilho de recuperação do núcleo disparou para o processo %d, mas o processo já liberou o núcleo que deveria.", process->id);
        LOG(DEBUG, "The core recovery trigger has been activated for the process %d, but the process has already released the core it should.", process->id);
        return;
    }
    //LOG(DEBUG, "O gatilho de recuperação do núcleo disparou para o processo %d. Movendo uma de suas threads para um núcleo não gerenciado.", process->id);
    LOG(DEBUG, "The core recovery trigger has been activated for the process %d. Moving one of its threads to the unmanaged core.", process->id);
    /**
     * Anota qual o núcleo onde foi preemptada para ser restaurada quando for despreemptada.
     * 
     * Note which core it was preempted to be restored when it is unpreempted.
     * */
    thread->corePreemptedFrom = thread->core;
    /**
     * Insere no mapa de processos o núcleo que estava executando e foi preemptado.
     * 
     * Acompanhamos, no nível do processo, os cores de onde as threads foram preemptionadas,
     * para que outra thread do mesmo processo não seja agendada para o core.
     * 
     * It inserts in the process map the core that was running and was preempted.
     * 
     * We follow, at the process level, the cores from which the threads were preempted,
     *  so that another thread of the same process is not scheduled for the core.
     * 
     * */
    thread->process->coresPreemptedFrom.insert(thread->core);
    /**
     * Remove a thread lenta dos núcleos gerenciados.
     * 
     * Removes the slow thread from managed cores.
     * */
    removeThreadFromManagedCore(thread);
    /**
     * Marca o status da thread preemptada como RUNNING_PREEMPTED.
     * 
     * Marks the status of the preempted thread as RUNNING_PREEMPTED.
     * */
    changeThreadState(thread, RUNNING_PREEMPTED);
    /**
     * Incrementa o contador de threads preemptadas.
     * 
     * Increments the preempted threads counter.
     * */
...
/**
 * Lista as threads que estão em execução e armazena o tempo que estão executando
 * através do seu identificador, caso seja identificada uma thread cujo o tempo  
 * em execução seja maior que um limite especificado ela será preemptada e função
 * distributeCores() será invocada para redistribuir os núcleos.
 * 
 * Por padrão o tempo é 2segundos, mas pode ser informado usando --timeSeconds <VALOR>
 * 
 * Caso uma thread seja preemptada o processo dono da thread será bloqueado, para que
 * outro processo tenha a oportunidade de escalonar threads.
 * 
 * A cada 10 execuções da função é feita uma limpeza do mapa listaThreadsExecutandoUM
 * sempre que uma thread é encerrada algumas entradas da thread encerrada podem se
 * manter no mapa desnecessariamente, então ele é limpo após 10 ciclos da função.
 * 
 * Lists the threads that are running and stores the time they are running 
 * through their identifier, if a thread is identified whose running time is 
 * greater than a specified limit, it will be preempted and the 
 * distributeCores() function will be invoked to redistribute the cores .
 * 
 * By default the time is 2 seconds, but it can be specified using --timeSeconds <VALUE>
 * 
 * If a thread is preempted, the process that owns the thread will be blocked, so that 
 * another process has the opportunity to schedule threads.
 * 
 * Every 10 executions of the function, the map listThreadsExecutandoUM 
 * is cleaned whenever a thread is terminated, some entries of the terminated thread 
 * may remain in the map unnecessarily, so it is cleaned after 10 cycles of the function.
 */
void CoreArbiterServer::listarThreadsArachne() {
    /***
     * Contador para limpar o mapa de resquícios de threads de sistema geradas pelo Arachne.
     * 
     * Counter for clearing the remnant map of system threads generated by Arachne.
    */
    if(timeToCleanUM < 10){
        timeToCleanUM++;
    }else{
        /**
         * Reinicia o gatilho de limpeza.
         * 
         * Resets the clear trigger.
         * */
        timeToCleanUM = 0;
        /**
         * Percorre o mapa de threads ativas e remove as entradas desnecessárias do mapa.
         * 
         * Goes through the map of active threads and removes unnecessary entries from the map.
         * */
        for (const auto& instanciaUM : listaThreadsExecutandoUM) {
            /**
             * Percorre o vetor de threads Arachne e compara com as entradas no mapa.
             * 
             * Loops through the vector of Arachne threads and compares them to entries in the map.
             * */
            for (const auto& instanciaTI : managedThreads) {
                /***
                 * Se a entrada no mapa não estiver no vetor, a entrada é removida do mapa.
                 * 
                 * If the map entry is not in the vector, the entry is removed from the map.
                */
                if (!(((int)instanciaUM.first) == ((int)instanciaTI->id))){
                    //std::cout << "Removendo Lixo ID:" << instanciaTI->id << "ProcessID: " << instanciaTI->process->id << std::endl;
                    //std::cout << "Removing Trash ID:" << instanciaTI->id << "ProcessID: " << instanciaTI->process->id << std::endl;
                    listaThreadsExecutandoUM.erase(instanciaUM.first);
                }
            }
        }
    }
    /**
     * Itera sobre as threads gerenciadas pelo CoreArbiterServer.
     * 
     * Iterates over threads managed by CoreArbiterServer.
    */
    for (ThreadInfo* thread : managedThreads) {
        /**
         * Verifica se a thread está com estado em execução - estados possíveis: RUNNING_MANAGED | RUNNING_UNMANAGED | RUNNING_PREEMPTED | BLOCKED 
         * 
         * Checks if the thread has a running state - possible states: RUNNING_MANAGED | RUNNING_UNMANAGED | RUNNING_PREEMPTED | BLOCKED
        */
        if (thread->state == RUNNING_MANAGED) { //RUNNING_MANAGED
            /**
             * Verifica se a thread está listada, se não estiver lista-a e armazena o tempo em que ela foi identificada.
             * 
             * Checks if the thread is listed, if not, lists it and stores the time it was identified.
            */
            if (listaThreadsExecutandoUM.count(thread->id)) { /* Se a thread estiver listada como executando - If the thread is listed as running */
                /**
                 * Marca à quanto tempo a thread está executando.
                 * 
                 * Marks how long the thread has been running.
                */
                uint64_t duration = std::chrono::duration_cast<std::chrono::nanoseconds>((std::chrono::high_resolution_clock::now()) - (listaThreadsExecutandoUM[thread->id])).count();
                /** 
                 * Checa se a thread está executando a mais tempo que o definido 2seg por padrão
                 * ou um valor informado usando a flag  --timeSeconds <VALOR> quando for invocado
                 * CoreArbiterServer para gerenciar as threads.
                 * 
                 * Checks if the thread has been running longer than the default 2sec 
                 * or a value set using the --timeSeconds <VALUE> flag when calling 
                 * CoreArbiterServer to manage threads.
                 * */
                if (duration > timeToPreempt){
                    //LOG(DEBUG, "Preemptar! thread->id: %u | thread->process->id: %d | Tempo decorrido: %lld nanossegundos", thread->id, thread->process->id, (long long int)duration);
                    LOG(DEBUG, "Preempt! thread->id: %u | thread->process->id: %d | Elapsed time: %lld nanoseconds", thread->id, thread->process->id, (long long int)duration);
                    //printf("Preemptar! thread->id: %u | thread->process->id: %d | Tempo decorrido: %lld nanossegundos\n", thread->id, thread->process->id, (long long int)duration);
                    /**
                     * Remove a Thread do mapa de threads em execução.
                     * 
                     * Remove Thread from the running thread map.
                    */
                    if(listaThreadsExecutandoUM.erase(thread->id)<=0){
                        //LOG(DEBUG, "ERRO ao remover do mapa de threads em execução! Thread Arachne: thread->id: %u | thread->process->id: %d | Tempo decorrido: %lld nanossegundos", thread->id, thread->process->id, (long long int)duration);
                        LOG(DEBUG, "ERROR removing from running thread map! Thread Arachne: thread->id: %u | thread->process->id: %d | Elapsed time: %lld nanoseconds", thread->id, thread->process->id, (long long int)duration);
                        //printf("ERRO ao remover do mapa de threads em execução! Thread Arachne: thread->id: %u | thread->process->id: %d | Tempo decorrido: %lld nanossegundos\n", thread->id, thread->process->id, (long long int)duration);
                    }else{
                        //LOG(DEBUG, "Sucesso ao remover do mapa de threads em execução! Thread Arachne: thread->id: %u | thread->process->id: %d | Tempo decorrido: %lld nanossegundos", thread->id, thread->process->id, (long long int)duration);
                        LOG(DEBUG, "Success removing from running thread map! Thread Arachne: thread->id: %u | thread->process->id: %d | Elapsed time: %lld nanoseconds", thread->id, thread->process->id, (long long int)duration);
                        //printf("Sucesso ao remover do mapa de threads em execução! Thread Arachne: thread->id: %u | thread->process->id: %d | Tempo decorrido: %lld nanossegundos\n", thread->id, thread->process->id, (long long int)duration);
                        
                        /**
                         * Preempta a Thread que foi listada como longa.
                         * 
                         * Preempt the Thread that was listed as long.
                        */
                        ThreadPreemption_portempo(thread);
                        /**
                         * Redistribui os núcleos entre aos demais processos.
                         * 
                         * Redistribute cores among other processes
                        */
                        distributeCores();
                    }
                }
                //sleep(1);
            } else{ /* Se a thread estiver executando e não tiver sido listada, cria uma entrada com seu id e o tempo atual - If thread is running and not listed, create an entry with its id and current time */
                listaThreadsExecutandoUM[thread->id] = std::chrono::high_resolution_clock::now();
                //printf("Armazenada! thread->id: %u - thread->process->id: %d\n", thread->id, thread->process->id);
                //sleep(1);
            }
        }
        /*
        else if (thread->state == RUNNING_UNMANAGED) { //RUNNING_UNMANAGED
            printf("RUNNING_UNMANAGED - threadid: %u - thread->process->id: %d\n", thread->id, thread->process->id);
        } else if (thread->state == RUNNING_PREEMPTED) { //RUNNING_PREEMPTED
            printf("RUNNING_PREEMPTED - threadid: %u - thread->process->id: %d\n", thread->id, thread->process->id);
        } else if (thread->state == BLOCKED) { //BLOCKED
            printf("BLOCKED - threadid: %u - thread->process->id: %d\n", thread->id, thread->process->id);
        }
        */
    }
}
